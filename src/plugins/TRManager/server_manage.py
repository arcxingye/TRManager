...
from .model import get_qq_tr, get_tr_isban, get_tr_qq, get_tr_reged, tr_add_ban,tr_add_user, tr_create_score, tr_del_ban, tr_del_user, tr_score_top, tr_sign_in, tr_update_score
...
async def SendTrRequest(alias: str, types: str, req: str):
    exec_url = ''
    if types == "cmd":
        exec_url = 'http://' + server_list.get(alias, "ip") + ':' + server_list.get(alias, "rest_port") + \
            '/v3/server/rawcmd?cmd='+req+'&token=' + \
            server_list.get(alias, "token")
    elif types == "ban":
        exec_url = 'http://' + server_list.get(alias, "ip") + ':' + server_list.get(alias, "rest_port") + \
            '/v2/players/ban?player='+req+'&token=' + \
            server_list.get(alias, "token")
    elif types == "inv":
        exec_url = 'http://' + server_list.get(alias, "ip") + ':' + server_list.get(alias, "rest_port") + \
            '/v3/players/read?player='+req+'&token=' + \
            server_list.get(alias, "token")
    try:
    ...
    if len(command)==4:
        ...
        if server and username and password:
            ...
            if not VerifyBan:
                            ...
                            if exec_result:
                                if exec_result['response'][0]=="User "+username+" already exists!":
                                    ...
                                    if insert_result:
                                        ...
                                        for group in tr_admin_list.get("tr_admin", "GROUP"):
                ...
                await tr_exec_reg.send("你已被封禁，无法进行注册。理由："+VerifyBan[2])
        else:
...
# 服管菜单
admin_menu = on_command("服管命令", priority=5, permission=GROUP)
@admin_menu.handle()
async def admin_menu_(bot: Bot, event: Event, state: T_State):
    if VerifyPermissions((str(event.get_session_id()).split("_"))[1], "GROUP"):
        await admin_menu.send("执行命令:/<服名> /<命令>\n全服执行:/全服执行 /<命令>\nban人:/<服名> !ban <角色名>\nqq号查角色:/qq查tr <qq>\n角色查qq:/tr查qq <服名> <角色名>\n加入黑名单:/trban <qq> <理由>\n解除黑名单:/bandel <qq>\n改积分:/设置积分 <qq> <分> <连续天数>")
        ...
        if result[0][2]>=0:
            ...
            if time_dif>0:
                if time_dif==1:
                    ...
                    count=1
                score_add=await tr_update_score(event.get_user_id(),int(result[0][2])+1,count)
                if score_add:
                    await sign_in.send("签到成功，积分+1\n现在拥有积分"+str(int(result[0][2])+1)+"\n已连续签到"+str(count)+"天")
                else:
